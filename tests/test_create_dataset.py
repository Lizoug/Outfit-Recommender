...
def test_create_dataset():
    ...
    sample_data = {'image_id': [1, 2, 3, 4, 5],
                   'articleType': ['Shirts', 'Jeans',
                                   'Track Pants', 'Tshirts',
                                   'Casual Shoes'],
                   'baseColour': ['Navy Blue', 'Blue',
                                  'Silver', 'Black', 'Grey']}
    ...
    # Check if the number of rows in the result == number of rows in the sample
    assert result.shape[0] == sample_df.shape[0]

    # Check if the articleType and baseColour columns only contain values
    # that are in the article_categories and color_names lists
    article_categories = ['Shirts', 'Jeans',
                          'Track Pants', 'Tshirts',
                          'Casual Shoes']
